""" Knowledge graph dataset for R-GCN
...
class RGCNLinkDataset(object):
    ...
    def load(self, load_time=True):   #在这里更改载入数据参数
        ...
        gridneighbor_path=os.path.join(self.dir, 'gridneighbor.txt')
        entityloc_neighbor_path=os.path.join(self.dir, 'entityloc_neighbor.txt')
        ...
        self.gridneighbor=np.array(_read_triplets_as_list(gridneighbor_path, entity_dict, relation_dict, load_time))
        self.entityloc_neighbor=np.array(_read_triplets_as_list(entityloc_neighbor_path, entity_dict, relation_dict, load_time))
